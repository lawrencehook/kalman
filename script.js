        ...
        // ================================
        // IMM Filter (2-model: slow vs fast process noise)
        // ================================
        class IMMFilter extends FilterBase {
            ...
            // helpers
            _vsub(a,b){ return a.map((row,i)=>[a[i][0]-b[i][0]]); }
            _outer(d){
                const o = Array(6).fill().map(()=>Array(6).fill(0));
                for(let i=0;i<6;i++) for(let j=0;j<6;j++) o[i][j]=d[i][0]*d[j][0];
                return o;
            }
            _madd(A,B){ return A.map((r,i)=>r.map((v,j)=>v+B[i][j])); }
            _mscale(A,s){ return A.map(r=>r.map(v=>v*s)); }

            // API
            getState(){ return this.state; }
            getPosition(){ return this.state ? [this.state[0][0], this.state[1][0]] : null; }
            getPositionCovariance(){ return this.covariance ? MatrixUtils.extractSubmatrix(this.covariance,0,1,0,1) : null; }
            getSystemMatrices(){
                const jStar = this.mu[0] >= this.mu[1] ? 0 : 1;
